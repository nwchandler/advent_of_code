...
fn part1(input: &str) -> Result<String, &'static str> {
    ...
    for line in input.lines() {
        ...
        let category = match categorize_hand(hand.as_str(), &None) {
...
fn part2(input: &str) -> Result<String, &'static str> {
    ...
    for line in input.lines() {
        let mut line_iter = line.trim().split_whitespace();
        // kind of hacky, but just replace J with 1 to signify its lower individual value
        let hand = convert_hand(line_iter.next().unwrap().replace('J', "1").as_str());
        let bid = line_iter.next().unwrap().parse::<u32>().unwrap();
        let hand_numeric_value = u32::from_str_radix(hand.as_str(), 16).unwrap();

        // because we replaced "J" with "1" up above, 1 is now our joker
        let category = match categorize_hand(hand.as_str(), &Some('1')) {
            HandType::FiveOfAKind => 6,
            HandType::FourOfAKind => 5,
            HandType::FullHouse => 4,
            HandType::ThreeOfAKind => 3,
...
fn categorize_hand(input: &str, joker: &Option<char>) -> HandType {
...
mod tests {
    ...
    fn test_categorize_hand() {
        // no jokers
        let mut joker: Option<char> = None;
        {
            // five of a kind
            let input = "AAAAA";
            let result = categorize_hand(input, &joker);
